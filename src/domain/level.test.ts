...
describe('asking about a cell outside the grid', () => {
  it('answers empty rather than failing, so callers need not bounds-check', () => {
    ...
    expect(elementAt(level, 0, 2)).toBeUndefined();
  });
});
...
describe('counting what clearing is waiting for', () => {
  ...
  it('is zero for a level with nothing to destroy', () => {
    expect(destructibleCount(levelFromRows(['pp-']))).toBe(0);
  });
});

describe("a level's authored bats", () => {
  it('places a horizontal bat on the row its glyph sat in', () => {
    const level = levelFromRows(['....', '.-..', '....']);

    expect(level.bats).toEqual([{ orientation: 'horizontal', line: 1, position: CELL_PIXELS }]);
  });

  it('places a vertical bat on the column its glyph sat in', () => {
    const level = levelFromRows(['....', '..|.', '....']);

    expect(level.bats).toEqual([{ orientation: 'vertical', line: 2, position: CELL_PIXELS }]);
  });

  it('leaves the cell a bat starts in empty, because a bat is not an element', () => {
    const level = levelFromRows(['.-..']);

    expect(elementAt(level, 1, 0)).toBeUndefined();
  });

  it('takes as many bats as were written', () => {
    const level = levelFromRows(['-..|', '....', '-...']);

    expect(level.bats).toHaveLength(3);
  });

  it('fails loudly on a level with no bat at all, which DS-1.3 forbids', () => {
    expect(() => levelFromRows(['...', 'ddd'])).toThrow(/at least one bat/);
  });
});
